...
namespace HotelBookingSystemAPI.Repositories
{
    public class RoomRepository : AbstractRepository<int,Room>
    {
        ...
        {
            var room = await _context.Rooms.Include(r => r.RoomType).Include(r=>r.roomsBooked).SingleOrDefaultAsync(r => r.RoomId == key);
            if (room == null)
                throw new ObjectNotAvailableException("Room");
            return room;
        }
        ...
        {
            var room = await _context.Rooms.Include(r => r.RoomType).Include(r => r.roomsBooked).ToListAsync();
            return room;
